 ...
 * It also has a static attribute <code>CHORD_SIZE</code> that tells us what the maximum key is in our system.
...
public class ChordState {
	...
	public void init(WelcomeMessage welcomeMsg) {
		...
		for (Map.Entry<String, VirtualFile> valueEntry : welcomeMsg.getValues().entrySet()) {
			VirtualFile virtualFile = valueEntry.getValue();
	...
	public static void quit(String serventIp, int serventPort){
		try {
			Socket bsSocket = new Socket(AppConfig.BOOTSTRAP_IP, AppConfig.BOOTSTRAP_PORT);
			PrintWriter bsWriter = new PrintWriter(bsSocket.getOutputStream());
			bsWriter.write("Adios\n" + serventIp + "\n" + serventPort + "\n");
			bsWriter.flush();
			bsSocket.close();
		} catch (IOException e) {
			e.printStackTrace();
